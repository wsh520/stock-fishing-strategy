...
3. **日线技术指标筛选**：低位深回调 + 右侧趋势转折 + 动能组确认 + 量价质量 + 波谷配对底背离；MA5拐头/EMA金叉在最近3个交易日内触发且当前短期结构仍有效即可，避免只认触发当天；MACD改善、RSI回升、KDJ低位转强归为同源动能组，至少满足一项且组内封顶计分，避免重复确认；底背离比较两个对应价格波谷的RSI/MACD并要求第二波谷后右侧确认。流动性过滤（近20日日均成交额 ≥500万）；入场质量否决（当日涨幅 >7%、开盘跳空高开 >3%、MA20近5日斜率 <-4%、RSI14 >65、量比 >5、距60日高点回撤 <10%）；现价仍须位于近20日价格区间下方40%以内
...
评分体系采用连续质量分：趋势质量(30) + 动能质量(25，MACD/RSI/KDJ组内封顶) + 低位质量(20) + 量价质量(15) + 波谷配对底背离(10)，满分100。趋势、动能、低位和量价根据斜率变化、ATR标准化强度、指标改善幅度、回撤/区间位置及量比区间连续计分，不再只有少数离散档位；多周期共振仅保留为诊断列，不重复加分。按分数分为 A/B/C/D（A≥80 / B≥60 / C≥40），`MIN_PASS_GRADE`默认B；此外必须具备最近3日内仍有效的趋势转折和至少一个动能组确认。底背离直接进入总分并在同分时优先排序，不再额外升档。熊市环境门槛提高10分。推荐排序：连续评分↓ → 底背离↓ → 近20日日均成交额↓ → 代码↑。

### 信号有效期与评分边界

- **三日趋势窗口**：每次MA5拐头/EMA金叉独立锚定触发日最低价；此后任何收盘价跌破该低点，永久撤销该次触发，价格回升不会复活旧触发。新的触发可以重建，且当前短趋势与MA20门控仍须通过。
- **底背离**：波谷右侧两根K线收盘后才允许使用该波谷，比较对应波谷当天指标。第二波谷后若日内最低价再次破低，撤销该配对，须等新波谷配对才能恢复。回看窗口按每个判定日距第一波谷的交易日距离计算（小于40日），历史判断不随输入总长度改变；第二波谷后的有效期仍为10日。
- **量价连续曲线**：上涨日使用 `S(x)=3x²−2x³`（x截断至[0,1]）。从正常量比 `VOLUME_NORMAL_RATIO=1.0` 到峰值 `VOLUME_PEAK_RATIO=2.5`，质量为 `S((量比−1)/(2.5−1))`；从峰值至 `MAX_VOL_RATIO=5.0`，质量为 `1−S((量比−2.5)/(5−2.5))`，端点归零。乘以15得到量价分；下跌/平盘日为0，超最大量比仍硬否决。正常量、峰值可配置且须满足0≤正常量<峰值<最大量比；旧1.2阈值仅用于放量诊断列，不再控制加分跳变。
- **动能成员**：RSI须当前至少上升 `DAILY_RSI_MIN_IMPROVEMENT=1.0` 点且处于35～65修复区间，前日反弹但今日回落不确认；KDJ须K>D、K≤60且当前K上行；MACD仍要求柱值当前改善。至少一项确认、组内25分封顶规则保留。
...
| 指标 | 环节 | 用途 |
|------|------|------|
| MA5/EMA5/10/20 | 日线 | 趋势转折（最近3日拐头/金叉有效，当前结构须保持转强） |
| MA20 斜率 | 日线 | 下降通道过滤（陡峭下降中趋势转折不认可，防接飞刀） |
| 当日涨幅/跳空幅度 | 日线 | 防追高否决（涨幅 >7% / 跳空高开 >3%） |
| RSI14 上限 / 量比上限 | 日线 | 已反弹一段否决（RSI>65）/ 天量出货否决（量比>5） |
| 60日高点回撤 / 20日区间位置 | 日线 | 低位硬过滤并按回撤深度、区间位置连续评分 |
| MACD柱 / RSI / KDJ | 日线 | 同源动能组至少一项确认，按改善强度组内封顶计分 |
...
| RSI14/7/21 | 日线 | 超卖修复、动能组确认及波谷配对背离判断 |
| 成交量比 | 日线 | 温和放量连续评分，异常天量否决 |
