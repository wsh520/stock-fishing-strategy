...
4. **风险收益比过滤**：固定止损止盈（止损 5% / 止盈 10%）+ 最低1.5倍风险收益比；**决赛圈周线趋势确认**（通过日线筛选的股票按评分降序逐个拉取周线，须站上周线 MA10（容忍2%）或 MA10 上行，取满即止）；最终推荐上限 `MAX_PICKS`（默认 5）只，目标每日 3~5 只精推
...
| 周线 MA10 | 周线 | 决赛圈趋势确认（站上周线MA10或MA10上行，仅对决赛圈拉取） |
