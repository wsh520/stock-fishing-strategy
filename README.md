...
3. **日线技术指标筛选**：底背离 + 趋势转折（MA5拐头/EMA金叉同源合并计分）+ RSI超卖反弹 + 量价配合；流动性过滤（近20日日均成交额 ≥500万）；入场质量否决（当日涨幅 >7% 追高否决、开盘跳空高开 >3% 否决、MA20 近5日斜率 <-4% 的陡峭下降通道中趋势转折信号不认可、RSI14 >65 已反弹一段否决、量比 >5 天量否决、距60日高点回撤 <10% 非底部区域否决）
4. **风险收益比过滤**：固定止损止盈（止损 5% / 止盈 10%）+ 最低1.5倍风险收益比；最终按评分降序截取前 `MAX_PICKS`（默认 5）只推荐，目标每日 3~5 只精推
