...
`StrategyConfig.RECOMMENDATION_MODE="quality_value"` 为默认，两个策略入口使用同一资格判定。这里的“低股价”指相对自身250个有效交易日区间的位置，不是每股面值低。

| 必选维度 | 默认规则 |
|---|---|
| 多年质量（非金融） | 最近连续3个可用完整年度：ROE中位数≥10%、每年ROE≥5%、每年扣非净利润>0、三年经营现金流合计/合并净利润合计≥0.8（分母须>0） |
| 估值 | PE与PB均须有限且齐全，0<PE TTM≤25、0<PB MRQ≤3；缺任一项只列待核验 |
...
只有显式`formal`会落库，pending会单独通知。两入口采用同一候选池，突破只是标签；已有同日去重继续避免重复推荐。年度缓存按代码、决策日期和年数隔离，首次运行会增加候选财务请求。

详细规则与测试见 [荐股调整说明](docs/quality_value_recommendations.md)。以下旧技术策略章节仅描述显式设置`RECOMMENDATION_MODE="technical"`时的对照行为，不代表当前默认模式；交易回测/追踪的原有局限并未因本次荐股调整自动消失。
