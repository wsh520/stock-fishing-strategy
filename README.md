...
3. **日线技术指标筛选**：底背离 + 趋势转折（MA5拐头/EMA金叉同源合并计分）+ RSI超卖反弹 + 量价配合；流动性过滤（近20日日均成交额 ≥500万）
4. **风险收益比过滤**：固定止损止盈（止损 5% / 止盈 10%）+ 最低1.5倍风险收益比

评分体系：趋势转折(40) + RSI反弹(25) + 量价配合(25) + 多周期共振(10) − RSI超买惩罚，满分 100。按分数分为 A/B/C/D 四个等级（A≥80 / B≥60 / C≥40，D级淘汰）；底背离个股评级额外提升一档（D 可被挽救为 C，与基础分脱钩避免分数通胀）。熊市环境等级门槛 +10 分。执行过程打印选股漏斗日志（各层通过率）。
